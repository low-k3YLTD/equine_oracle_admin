...
export class DataValidator {
  ...
  /**
   * Validate CSV horse data for batch predictions
   */
  static validateCSVData(csvContent: string): {
    isValid: boolean;
    rows: any[];
    errors: string[];
    warnings: string[];
    ...
    try {
      ...
      if (lines.length < 2) {
        errors.push('CSV file must contain at least header and one data row');
        return { isValid: false, rows: [], errors, warnings };
      }

      // Parse header
      const headers = lines[0].split(',').map(h => h.trim());
      const requiredHeaders = ['horseName', 'distanceNumeric', 'daysSinceLastRace'];
      ...
      for (let i = 1; i < lines.length; i++) {
        ...
        // Validate row
        if (!row.horseName) {
          errors.push(`Row ${i + 1}: Missing horse name`);
          continue;
        }

        if (!row.distanceNumeric || isNaN(parseFloat(row.distanceNumeric))) {
          errors.push(`Row ${i + 1}: Invalid distance value`);
          continue;
        }

        if (!row.daysSinceLastRace || isNaN(parseInt(row.daysSinceLastRace))) {
          errors.push(`Row ${i + 1}: Invalid days since last race`);
          continue;
        }
